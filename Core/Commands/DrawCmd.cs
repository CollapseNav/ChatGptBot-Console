...
public class DrawCmd : ClassificationCmd
{
    ...
    public DrawCmd(BaseChatSession session, IConfig<SdConfig> sdconfig, IConfig<NlpConfig> nlpconfig, HttpClient client)
    {
    ...
    public override async Task<bool> ExecAsync(IBotMsg botMsg)
    {
        ...
        - Kanibot在生成优化的描述后，会在之后添加描述 图片的关键词。Kanibot会以关键词形式单独描述图片中出现的每一个对象，以及增加对于主题、背景、色调和样式等抽象概念的关键字 ，来丰富画面的细节。Kanibot可以自由选择画面中应有的细节添加关键词。Kanibot会在每两个关键词中间加上逗号，表示关键词之间的分隔符。这将组成prompt的第三部分。
        ...
        {botMsg.Msg}
        """;
        var aiPrompt = await session.AskAsync(quesiton);
        var prompts = aiPrompt.Split(',', '.', '|').Select(item => item.Trim()).Where(item => item.NotEmpty()).ToArray();
        var qualityPropmts = new[] { "(masterpiece)", "best quality", "highly detailed", "ultra-detailed", };
        var negativePrompts = "(worst quality, low quality:1.4), monochrome, zombie, (nsfw:1.4), (nude:1.4), wrong hand,";
        prompts = qualityPropmts.Concat(prompts).Unique().ToArray();
